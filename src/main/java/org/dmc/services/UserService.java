...
public class UserService {
	...
	public Boolean canCreateOrg(Integer userId) throws DMCServiceException {
		UserRoleAssignmentRO ura = userRoleAssignmentRepository.findByUserId(userId);
		if(ura != null && SecurityRoles.SUPERADMIN.equalsIgnoreCase(ura.getRole().getRole())) {
			return true;
		}
		OrganizationUser orgUser = orgUserService.getOrgUserEntityByUserId(userId);
		if(orgUser != null) {
			if (orgUser.getIsVerified()) {
				throw new DMCServiceException(DMCError.UnauthorizedAccessAttempt,
						"You tried to create an organization while verified with an organization already, "
