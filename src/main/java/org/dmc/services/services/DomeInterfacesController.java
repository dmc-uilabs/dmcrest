...
public class DomeInterfacesController {
	...
	private static final String LOGTAG = DomeInterfacesController.class.getName();
	private final DomeInterfacesDao domeInterfacesDao = new DomeInterfacesDao();
	...
	@RequestMapping(value = "/{domeInterfaceId}", produces = { APPLICATION_JSON_VALUE }, method = RequestMethod.DELETE)
	...
	public ResponseEntity domeInterfacesDomeInterfaceIdGet(@PathVariable("domeInterfaceId") BigDecimal domeInterfaceId) {
		...
		try {
			final GetDomeInterface read = domeInterfacesDao.getDomeInterface(domeInterfaceId);
			if (read == null) {
