...
public class IndividualDiscussionDao {
	...
	public List<IndividualDiscussion> getListOfCommunityIndividualDiscussions(Integer limit, String order, String sort) throws DMCServiceException {
		...
		try {
			...
			columnsInIndividualDiscussionTable.add("accountId");
			columnsInIndividualDiscussionTable.add("projectId");
			...
			while (resultSet.next()) {
				if (new BigDecimal(resultSet.getInt("projectId")) != null) {
					...
					retObj.setCreatedAt(new BigDecimal(resultSet.getInt("created_at")));
