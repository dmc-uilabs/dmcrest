...
public class ComponentController {
	 ...
	 * 
	 * @ResponseBody public String createRole(@RequestParam(value="name",
	 * defaultValue="anonymous") String name) { System.out.println(
	 * "In createRole role: " + name);
	 * 
	 * 
	 * //RoleDao.createRole creates a new Role in the database using the
	 * provided POST params //it instantiates a new role with these params like
	 * i.e new Role(param.name, param.title.....) //this controller in turn
	 * returns this new Role instance to the reques using spring's Jackson which
	 * //converts the response to JSON
	 * 
	 * return RoleDao.createRole(params);
	 * 
	 * //Create role and update db through JDBC then return role using new
	 * role's id
	 * 
	 * if (true) { return "Could not create role"; //return new
	 * ResponseEntity<Role>(HttpStatus.BAD_REQUEST); } //return new
	 * ResponseEntity<String>(json,HttpStatus.OK); return
