...
public class CompareServiceIT extends BaseIT {
	...
	public void postAndDeleteCompareService() {
		...
		GetCompareService compareService = given().header("Content-type", "application/json")
				.header("AJP_eppn", forgeAdmin).body(json).expect().statusCode(HttpStatus.OK.value()).when()
	...
	public void deleteCompareService(int id) {
		given().header("Content-type", "application/json").header("AJP_eppn", forgeAdmin).expect()
	...
	public void getCompareServicesWithInvalidProfileId() {
		int profileId = 102;
		given().header("AJP_eppn", userEPPN).expect().statusCode(HttpStatus.UNAUTHORIZED.value()).when()
				.get(PROFILE_COMPARE_SERVICE, profileId);
	}
